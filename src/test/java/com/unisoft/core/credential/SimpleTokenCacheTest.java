package com.unisoft.core.credential;
...
//                            Thread.currentThread().getName(), Duration.between(start, OffsetDateTime.now()).toMillis());
    ...
    private Mono<AccessToken> remoteGetTokenAsync(long delayInMillis) {
        return Mono.delay(Duration.ofMillis(delayInMillis))
                .map(l -> new TestToken(Integer.toString(RANDOM.nextInt(100))));
    }

    private Mono<AccessToken> remoteGetTokenThatExpiresSoonAsync(long delayInMillis, long validityInMillis) {
        return Mono.delay(Duration.ofMillis(delayInMillis))
                .map(l -> new TestToken(Integer.toString(RANDOM.nextInt(100)), validityInMillis));
    }
    ...
    private static class TestToken extends AccessToken {
        private final String token;
        private final OffsetDateTime expiry;

        TestToken(String token) {
            this(token, 5000);
        }

        TestToken(String token, long validityInMillis) {
